...
for (const p of PRODUCTS) {
  ...
  if (p.status === "live") {
    ...
    // sidebar is optional — a product without one uses the shared
    // autogenerated sidebar, so only validate the file when declared.
    const sidebar = p.sidebar ?? "./sidebars/autogen.js";
    if (!fs.existsSync(sidebar.replace("./", ""))) {
      errors.push(`${p.id}: sidebar "${sidebar}" does not exist`);
    }
